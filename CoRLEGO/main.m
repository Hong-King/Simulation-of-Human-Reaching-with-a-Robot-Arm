...
angleLimit = 3*pi/2;        % the joint limit, used to control the position and orientation of randomly generated targets
...
% Interference Target 1
q_des2 = rand(DOF,1) * angleLimit;              % desired joint configuration
[T,~] = FK(DH, jtype, q_des2);
p_des2 = T(1:3,4);               % desired end-effector position
R = T(1:3,1:3);
phi_des2 = r2rpy(R);          % desired end-effector orientation

% Interference Target 2
q_des3 = rand(DOF,1) * angleLimit;              % desired joint configuration
[T,~] = FK(DH, jtype, q_des3);
p_des3 = T(1:3,4);               % desired end-effector position
R = T(1:3,1:3);
phi_des3 = r2rpy(R);          % desired end-effector orientation


%% call CommonMethod function
[Q1,waypoints1,loopPoints1] = CoRLEGO(DH, jtype, q, p_des1, phi_des1,p_des2, p_des3, runningTime);
...
plot3(p_des2(1),p_des2(2),p_des2(3),'o',MarkerFaceColor='g');
plot3(p_des3(1),p_des3(2),p_des3(3),'o',MarkerFaceColor='b');

[TCur_k,~] = FK(DH, jtype, q);
pIni = TCur_k(1:3,4);
plot3(pIni(1),pIni(2),pIni(3),'o',MarkerFaceColor='y');
legend('CoRLEGO','Loop Points','Target','interferer 1', 'interferer 2', 'Initial Point'); % ,'Outer Loop'
hold off
